...
class OpsRegistryTest(unittest.TestCase):
    ...
    def test_cli_commands_json_describes_installed_command_surface(self):
        ...
        self.assertIn("1. mechferret init", rendered_commands)
        self.assertIn("2. mechferret quickstart --run", rendered_commands)
        self.assertIn("5. mechferret support", rendered_commands)
        self.assertIn("5. mechferret verify-bundle --select best --strict", rendered_commands)
        self.assertNotIn(" -> ...", rendered_commands)
    ...
    def test_cli_api_json_redacts_keys_and_reports_updates(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertNotIn("next_actions", shown)
                self.assertIn("mechferret login openai", " ".join(shown["suggested_next_actions"]))
                ...
                self.assertNotIn("next_actions", updated)
                self.assertIn("mechferret login anthropic", " ".join(updated["suggested_next_actions"]))
                ...
                self.assertNotIn("next_actions", cleared)
                self.assertIn("mechferret login openai", " ".join(cleared["suggested_next_actions"]))
    ...
    def test_run_quickstart_demo_creates_passing_local_dossier(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("mechferret open report --select latest --browser", " ".join(report["next_actions"]))
            self.assertIn("mechferret audit --select latest --strict", " ".join(report["next_actions"]))
            ...
            self.assertIn("mechferret review-paper --select latest", " ".join(paper["next_actions"]))
            self.assertIn("mechferret paper --select latest --compile", " ".join(paper["next_actions"]))
    ...
    def test_project_status_summarizes_latest_run_and_actions(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(status["artifact_summary"]["groups"]["run"]["total"], 6)
            self.assertEqual(status["artifact_summary"]["groups"]["setup"]["total"], 3)
            self.assertIn("artifact_readiness", status)
            self.assertTrue(status["artifact_readiness"]["run"]["ok"])
    ...
    def test_resolve_artifact_exposes_discovery_outputs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(index["artifact_readiness"]["run"]["ok"])
            self.assertFalse(index["artifact_readiness"]["sharing"]["ok"])
            self.assertFalse(index["complete"])
            self.assertTrue(index["run_ready"])
            self.assertFalse(index["share_ready"])
    ...
    def test_resolve_openvla_accepts_ready_scaffold_without_generated_index(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("mechferret sae openvla status", " ".join(resolved["next_actions"]))
            self.assertIn("mechferret sae openvla commands", " ".join(resolved["next_actions"]))
    ...
    def test_cli_discover_json_requires_explicit_backend(self):
        from mechferret.cli import main
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(any("--backend" in action for action in payload["next_actions"]))
