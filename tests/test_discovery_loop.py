...
class DiscoveryLoopTest(unittest.TestCase):
    def test_full_loop_finds_reproducible_mechanism(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(run.provenance["seed_source"], "run_generated")
            self.assertEqual(len(run.provenance["seeds"]), 3)
            self.assertNotEqual(run.provenance["seeds"], [0, 1, 2])
            self.assertTrue(all(len(experiment.per_seed) == len(run.provenance["seeds"]) for experiment in run.experiments))
