...
SECTIONS: list[tuple[str, list[Command]]] = [
    ("Chat", [
        ...
        Command("/queue tail [id|latest|active|running|side] [seconds|now]", "follow live output, or use now for a non-blocking snapshot"),
        ...
        Command("/queue restore [id|latest|running|side|next|all]", "restore saved queued/running prompts from the last session"),
        Command("/queue wait [seconds]", "block until active queued and side work finishes"),
        Command("/queue join <id|latest|active|running|side|next> [seconds]", "wait for one queued or side job to finish"),
