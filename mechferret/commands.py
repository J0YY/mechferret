...
SECTIONS: list[tuple[str, list[Command]]] = [
    ("Chat", [
        ...
        Command("/queue show <id|latest>", "show a queued job's full prompt, reply, or error"),
        Command("/queue retry <id|latest>", "retry a job without retyping its prompt"),
        Command("/queue edit <id|latest> <text>", "edit a queued prompt before it starts"),
        Command("/queue move <id|latest> first|last|before|after", "reorder a queued prompt before it starts"),
        Command("/queue cancel <id|latest|all>", "cancel queued prompts from the queue view"),
        ...
        Command("/queue join <id> [seconds]", "wait for one queued or side job to finish"),
