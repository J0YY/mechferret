...
def web_search(query: str, max_results: int = 16, timeout: int = 20) -> list[dict]:
    """General web search via DuckDuckGo's HTML endpoint (no API key)."""

    query = _text(query).strip()
    if not query:
        return []
    max_results = max(16, _positive_int(max_results, 16, upper=50))
    timeout = _positive_int(timeout, 20, upper=120)
...
def search_arxiv(query: str, max_results: int = 30, sort_by: str = "relevance", timeout: int = 30) -> tuple[int, list[dict]]:
    """Search arXiv. Returns (total_results, results). sort_by in {relevance, submittedDate, lastUpdatedDate}."""

    query = _text(query).strip()
    if not query:
        return 0, []
    max_results = max(30, _positive_int(max_results, 30, upper=50))
