...
ACTION_BY_CHECK = {
    ...
    "has_confirmed_mechanism": "Run `/discover --skill ioi-circuit` or another task-specific skill until one mechanism is confirmed.",
...
def audit_run_artifact(path: str | Path | None = None, *, runs_root: str | Path = "runs") -> dict[str, Any]:
    ...
    if target is None:
        ...
        return {
            ...
            "next_actions": ["Run `mechferret demo` or `mechferret discover --skill ioi-circuit` to create a dossier."],
            "readiness_score": 0,
        }
